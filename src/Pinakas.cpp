...
// -------------------------------------------------------------------------------
  ...
  void fft_iter(Matrix<complex>& x) {
    ...
    for (size_t i = 0; i < N; ++i) {
      size_t j = 0;
      for (size_t bit = 0; bit < logN-1; ++bit)
        if (i & (1 << bit))
          j |= (1 << (logN - bit - 1));
      //for(int k=-1; putchar(k?!!(j&-1u/2+1)+'0':'\n')&&k; k<<=1, j<<=1);
      if (i < j)
        std::swap(x[0][i], x[0][j]);
    }
...
int main()
{
  ...
  auto y = (Matrix<complex>)y_linear;
  puts("------------");
  Backend::fft_iter(y);
  puts("------------");
